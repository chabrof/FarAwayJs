...
export class SimpleStream {
    constructor(host = "localhost", port = "8080", calleeSecureHash) {
        this._treat = {};
        this._myCallerGUID = new Chance.Chance().guid();
        this.addEventListener = function (cbk) {
            _console.log('addEventListener');
            _console.assert(cbk && typeof cbk === "function", "Arg 'cbk' must be provided (function)");
            let listener = {
                cbk: cbk,
                arrayIdx: null
            };
        ...
        this.removeEventListener = function (listenerIdx) {
            // pre
            _console.assert(listenerIdx !== undefined && listenerIdx !== null && listenerIdx >= 0, "listenerIdx must be a not null integer");
    ...
    init() {
        ...
        let declareWsReady;
        let openingPromise = new Promise((ok, ko) => declareWsReady = ok);
        ...
        let finalPromise = new Promise((ok, ko) => {
    ...
    _treatFarHandShakeReturn(callObj) {
