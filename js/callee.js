...
let _callables = {};
let _rCallIdx = 0;
let _com, _comReadyPromise;
let _instancesH = {};
let _magicToken = new Chance.Chance().guid();
let _callerSecureHashes = {};
let _myCalleeSecureHash = generateSecureHash(_magicToken, new Chance.Chance().guid());
let _instanceIdx = -1;
let setCommunication = function (communication) {
    _com = communication;
...
function register(object, name = undefined, excludeCalls = undefined) {
...
export class CallableObject {
    constructor(name, object, type, excludeCalls = []) {
        this.structure = {};
        this.name = name;
        this.type = type;
        this.object = object;
...
let _checkSecureHash = function (callerSecureHash, instanceIdx) {
...
_treat.farImport = function (callObj) {
    ...
    let result = [];
    callObj.symbols.forEach((symbol) => {
